...
public class DistributeLock implements Lock, Watcher {
    ...
    //等待前一个锁，相对于CURRENT_LOCK，那么前一个节点就是WAIT_LOCK
    ...
    public DistributeLock() {
        try {
            ...
            zooKeeper = new ZooKeeper("1.14.163.139:2181", 4000, this);
            //exists判断根节点是否存在，含有注册事件 。watch： false .因为不需要再注册对这个节点的事件。
            Stat stat = zooKeeper.exists(ROOT_LOCK, false);
            if (stat == null) {
                //创建根节点
                zooKeeper.create(ROOT_LOCK, "0".getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE,
    ...
    private boolean waitForLock(String prev) {
        try {
            //监听当前节点的上一个节点
            Stat stat = zooKeeper.exists(prev, true);//因为它没有获得锁，所以就需要监听上一个节点的状态
            if (stat != null) {
